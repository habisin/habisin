<x-app-layout>
    <div class="max-w-4xl mx-auto py-8">
        ...
        @if(!request('lat') || !request('lng'))
            <div id="locationCard" class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-xl shadow">
                ...
                <p class="text-sm text-blue-700 mt-1">
                    HabisIn memerlukan akses lokasi Anda untuk menampilkan toko terdekat dan membantu Anda menemukan makanan
                    dengan lebih cepat.
                </p>
        ...
        <form method="GET" action="{{ route('buyer.shops') }}" class="flex gap-3 mb-6">
            <input name="search" value="{{ request('search') }}" type="text" placeholder="Cari toko..."
                class="w-full p-3 border rounded-lg">

            <select id="filterSelect" class="p-3 border rounded-lg" onchange="filterShops()">
        ...
        <div id="shopList" class="space-y-4">
            @foreach($shops as $shop)
                <a href="{{ route('buyer.shops.show', $shop->id) }}"
                    class="block hover:bg-gray-50 active:scale-[0.99] transition rounded-xl">
                    <div class="shop-card flex gap-4 p-4 bg-white rounded-xl shadow"
                        data-name="{{ strtolower($shop->name) }}" data-description="{{ strtolower($shop->description) }}">
